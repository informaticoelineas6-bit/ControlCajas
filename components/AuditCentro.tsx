...
const defaultCentroAudit: CentroAudit = {
  centro: {
...
export default function AuditCentro() {
  ...
  const [datos, setDatos] = useState<CentroAudit>(defaultCentroAudit);
  ...
  const fetchCentros = async (signal: AbortSignal) => {
    ...
    try {
      const res = await fetch("/api/form/centros", { signal });
      const data = await res.json();
      ...
      <div className="border-b border-slate-200 bg-[linear-gradient(135deg,_rgba(245,158,11,0.16),_rgba(255,255,255,0.96))] px-6 py-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          ...
          <span className="rounded-full bg-white px-4 py-2 text-sm font-medium text-slate-600 shadow-sm ring-1 ring-slate-200">
            {loading ? "Cargando..." : `${centros.length} disponibles`}
          </span>
        </div>
      </div>
      ...
      <div className="space-y-6 p-6">
        ...
        <div className="space-y-3 lg:hidden">
          <article
            ...
            className="rounded-2xl text-center border border-amber-100 bg-amber-50 p-4 shadow-sm"
          >
            ...
            <div className="mt-4 grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
              ...
              <div>
                ...
                <p className="font-medium text-slate-700">
                  {datos.centro.ajuste ? prettyName(datos.centro.ajuste) : "-"}
                </p>
              </div>
            </div>
          </article>
