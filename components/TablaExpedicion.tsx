...
export default function TablaExpedicion({
  ...
  useEffect(() => {
    ...
    const channel = frontendClient
      .channel(`${TABLAS.EXPEDICION}_changes`)
      .on(
        "postgres_changes",
  ...
  return (
    <section className="overflow-hidden rounded-[28px] border border-slate-200/80 bg-white/95 shadow-[0_24px_60px_-34px_rgba(15,23,42,0.35)]">
      <div className="border-b border-slate-200 bg-[linear-gradient(135deg,_rgba(34,197,94,0.1),_rgba(255,255,255,0.96))] px-6 py-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.32em] text-slate-500">
              Salida desde almacén
            </p>
            ...
            datos.map((item) => (
              ...
              >
                ...
                <div className="mt-4 grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                  ...
                  <div>
                    <p className="text-slate-600">Expedidor</p>
                    <p className="font-medium text-slate-700">
          ...
          <table className="min-w-full text-sm text-center">
            <thead className="bg-slate-50 text-slate-800">
              <tr>
                ...
                <th className="px-5 py-4 font-semibold">Expedidor</th>
                {CAJAS_ARRAY.map((color: COLORES_CAJAS) => (
                  <th
                    key={color}
                    className={
