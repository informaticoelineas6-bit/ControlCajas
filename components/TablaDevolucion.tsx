...
export default function TablaDevolucion({
    ...
    async (signal: AbortSignal) => {
      ...
      try {
        const res = await fetch(
          `/api/eventos/list?fecha=${fecha}&tipo=${"Devolucion" as TIPOS_EVENTO}`,
          { signal },
        );
  ...
  useEffect(() => {
    ...
    const channel = frontendClient
      .channel(`${TABLAS.DEVOLUCION}_changes`)
      .on(
        "postgres_changes",
            ...
            datos.map((item) => (
              ...
              >
                ...
                <div className="mt-4 grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                  ...
                  <div>
                    <p className="text-slate-600">Almacenero</p>
                    <p className="font-medium text-slate-700">
          ...
          <table className="min-w-full text-sm text-center">
            <thead className="bg-slate-50 text-slate-800">
              <tr>
                ...
                <th className="px-5 py-4 font-semibold">Almacenero</th>
                {CAJAS_ARRAY.map((color: COLORES_CAJAS) => (
                  <th
                    key={color}
                    className={
